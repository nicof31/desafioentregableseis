...
routerView.get("/", sessionExist, async (req, res) => {
  try {
    ...
    res.render("user/loginerror", { error: "Ocurrió un error al renderizar la página de inicio" });
  }
});
...
routerView.get("/login", sessionExist, async (req, res) => {
  try {
    ...
    res.render("user/loginerror", { error: "Ocurrió un error al renderizar la página de login" });
  }
});

//-------------------------GUARDO LOS DATOS DE LA SESION EN MONGO------------------------------------------//
  routerView.use(
    session({
